...
const s3Client = new S3Client({
  ...
  endpoint: `https://s3.${process.env.MY_REGION}.amazonaws.com`,
  credentials: {
...
export async function POST(req: Request) {
  try {
    ...
    const fileUrl = `https://${process.env.MY_S3_BUCKET_NAME}.s3.${process.env.MY_REGION}.amazonaws.com/${fileKey}`;
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to retrieve images', details: error },
      { status: 500 }
    );
  }
}
